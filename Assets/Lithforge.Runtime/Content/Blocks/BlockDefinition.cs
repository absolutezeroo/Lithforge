...
namespace Lithforge.Runtime.Content.Blocks
{
    ...
    {
        [FormerlySerializedAs("_namespace"), Header("Identity"), Tooltip("Namespace for the resource id (e.g. 'lithforge')"), SerializeField]
        ...
        [FormerlySerializedAs("_blockName"), Tooltip("Block name (e.g. 'stone')"), SerializeField]
         private string blockName = "";

        [FormerlySerializedAs("_hardness"), Header("Gameplay"), Tooltip("Time to break in seconds"), Min(0f), SerializeField]
        ...
        [FormerlySerializedAs("_blastResistance"), Tooltip("Resistance to explosions"), Min(0f), SerializeField]
         private double blastResistance = 1.0;

        /// <summary>When true, mining with a weaker tool than required still breaks the block but yields no drops.</summary>
        [FormerlySerializedAs("_requiresTool"), Tooltip("Whether a tool is required to get drops"), SerializeField]
         private bool requiresTool;

        /// <summary>
        ///     Physical material category — determines which tool type mines fastest (pickaxe for stone, shovel for dirt,
        ///     etc.).
        /// </summary>
        [FormerlySerializedAs("_materialType"), Tooltip("Physical material of the block (determines mining speed per tool)"), SerializeField]
         private BlockMaterialType materialType = BlockMaterialType.Stone;

        /// <summary>Minimum tool tier needed to get drops (0 = hand, 1 = wood, 2 = stone, 3 = iron, 4 = diamond).</summary>
        [FormerlySerializedAs("_requiredToolLevel"), Tooltip("Minimum tool level required (0 = none, 1 = wood, 2 = stone...)"), Min(0), SerializeField]
         private int requiredToolLevel;

        /// <summary>Sound group name for break/place/step audio (e.g. "stone", "wood", "grass").</summary>
        [FormerlySerializedAs("_soundGroup"), Tooltip("Sound group for block sounds"), SerializeField]
         private string soundGroup = "stone";

        /// <summary>AABB shape used by VoxelCollider. None = passthrough (air, torches).</summary>
        [FormerlySerializedAs("_collisionShape"), Header("Physics"), Tooltip("Collision shape type"), SerializeField]
         private CollisionShapeType collisionShape = CollisionShapeType.FullCube;

        /// <summary>Rendering pass — controls submesh assignment and alpha handling.</summary>
        [FormerlySerializedAs("_renderLayer"), Header("Rendering"), Tooltip("Render layer for transparency sorting"), SerializeField]
         private RenderLayerType renderLayer = RenderLayerType.Opaque;

        /// <summary>Marks this as a fluid block (water, lava). Lowers the top face by 0.125 blocks for the fluid surface effect.</summary>
        [FormerlySerializedAs("_isFluid"), Tooltip("Whether this block is a fluid (water, lava) — affects top-face height"), SerializeField]
        ...
        [FormerlySerializedAs("_defaultTintType"), Tooltip("Default biome tint type applied to all faces when the model has no per-face tintIndex. " +
                                                           ...
                                                           "2 = foliage colormap, 3 = water (per-biome color)"), Range(-1, 3), SerializeField]
         private int defaultTintType = -1;

        /// <summary>Block light emission level (0–15). Torch = 14, glowstone = 15.</summary>
        [FormerlySerializedAs("_lightEmission"), Tooltip("Light emitted by this block (0-15)"), Range(0, 15), SerializeField]
         private int lightEmission;

        /// <summary>Light absorbed when passing through this block (0–15). 15 = fully opaque, 0 = transparent.</summary>
        [FormerlySerializedAs("_lightFilter"), Tooltip("Light absorbed by this block (0-15)"), Range(0, 15), SerializeField]
         private int lightFilter = 15;

        /// <summary>Hex color for the world map display (#RRGGBB or #RRGGBBAA).</summary>
        [FormerlySerializedAs("_mapColor"), Tooltip("Color shown on the map (#RRGGBB or #RRGGBBAA)"), SerializeField]
         private string mapColor = "#808080";

        /// <summary>Loot table rolled when this block is broken. Null means no drops.</summary>
        [FormerlySerializedAs("_lootTable"), Header("References"), Tooltip("Loot table for this block"), SerializeField]
         private LootTable lootTable;

        /// <summary>Maps property combinations to model variants (e.g. axis=y → upright log model).</summary>
        [FormerlySerializedAs("_blockStateMapping"), Tooltip("Block state mapping (variants)"), SerializeField]
        ...
        /// </summary>
        [FormerlySerializedAs("_properties"), Header("Properties"), Tooltip("Block state properties (axis, facing, lit, etc.)"), SerializeField]
         private List<BlockPropertyEntry> properties = new();

        /// <summary>Tag string IDs this block belongs to (e.g. "mineable_pickaxe", "logs").</summary>
        [FormerlySerializedAs("_tags"), Header("Tags"), Tooltip("Tags this block belongs to (string ids for backward compatibility)"), SerializeField]
