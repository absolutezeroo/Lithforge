...
namespace Lithforge.Runtime.Content.Items
{
    ...
    ///     Loaded by ContentPipeline and registered into <see cref="ItemRegistry" />.
    ...
    public sealed class ItemDefinition : ScriptableObject
    {
        ...
        [FormerlySerializedAs("_itemName"), Tooltip("Item name"), SerializeField]
         private string itemName = "";

        /// <summary>Maximum stack size in a single inventory slot (1 for tools, 64 for materials).</summary>
        [FormerlySerializedAs("_maxStackSize"), Header("Stack"), Tooltip("Maximum stack size"), Range(1, 64), SerializeField]
         private int maxStackSize = 64;

        /// <summary>Which tool category this item belongs to (None, Pickaxe, Axe, Shovel, Sword, Hoe).</summary>
        [FormerlySerializedAs("_toolType"), Header("Tool Properties"), Tooltip("Tool type"), HideInInspector, SerializeField]
        ...
        [FormerlySerializedAs("_toolLevel"), Tooltip("Tool mining level"), Min(0), HideInInspector, SerializeField]
         private int toolLevel;

        /// <summary>Total durability before the tool breaks. 0 = indestructible.</summary>
        [FormerlySerializedAs("_durability"), Tooltip("Item durability (0 = unbreakable)"), Min(0), HideInInspector, SerializeField]
         private int durability;

        /// <summary>Base damage dealt to entities on hit.</summary>
        [FormerlySerializedAs("_attackDamage"), Tooltip("Attack damage"), Min(0f), HideInInspector, SerializeField]
         private float attackDamage = 1.0f;

        /// <summary>Base mining speed multiplier.</summary>
        [FormerlySerializedAs("_miningSpeed"), Tooltip("Mining speed multiplier"), Min(0f), HideInInspector, SerializeField]
         private float miningSpeed = 1.0f;

        /// <summary>Passive modifiers applied while this item is held (e.g. efficiency, fortune).</summary>
        [FormerlySerializedAs("_affixes"), Tooltip("Tool affixes"), SerializeField]
         private AffixDefinition[] affixes
            = Array.Empty<AffixDefinition>();

        /// <summary>Burn time in seconds when used as furnace fuel. 0 = not a fuel item.</summary>
        [FormerlySerializedAs("_fuelTime"), Header("Fuel"), Tooltip("Fuel burn time in seconds (0 = not a fuel item)"), Min(0f), SerializeField]
         private float fuelTime;

        /// <summary>Block placed when this item is used. Null for non-block items (tools, materials).</summary>
        [FormerlySerializedAs("_placesBlock"), Header("Block Placement"), Tooltip("Block this item places when used"), SerializeField]
         private BlockDefinition placesBlock;

        /// <summary>3D model used for held-item rendering and item entity display.</summary>
        [FormerlySerializedAs("_itemModel"), Header("Model"), Tooltip("Item model reference"), SerializeField]
         private BlockModel itemModel;

        /// <summary>Tag string IDs this item belongs to.</summary>
        [FormerlySerializedAs("_tags"), Header("Tags"), Tooltip("Tags this item belongs to"), SerializeField]
