...
namespace Lithforge.Runtime.Content.Loot
{
    ...
    public sealed class LootPoolEntry
    {
        ...
        [FormerlySerializedAs("_rollsMin"), Tooltip("Minimum rolls"), Min(0), SerializeField]
         private int rollsMin = 1;

        /// <summary>Maximum number of times this pool is rolled (inclusive).</summary>
        [FormerlySerializedAs("_rollsMax"), Tooltip("Maximum rolls"), Min(0), SerializeField]
         private int rollsMax = 1;

        /// <summary>Weighted-random entries — one is selected per roll.</summary>
        [FormerlySerializedAs("_entries"), Tooltip("Entries in this pool"), SerializeField]
         private List<LootItemEntry> entries = new();

        /// <summary>All conditions must pass for this pool to be rolled at all.</summary>
        [FormerlySerializedAs("_conditions"), Tooltip("Conditions for this pool"), SerializeField]
