...
namespace Lithforge.Runtime.Content.Loot
{
    ...
    public sealed class LootItemEntry
    {
        ...
        [FormerlySerializedAs("_type"), Tooltip("Entry type (item, empty, loot_table)"), SerializeField]
         private string type = "item";

        /// <summary>Direct item reference. Preferred over itemName when available.</summary>
        [FormerlySerializedAs("_item"), Tooltip("Item reference"), SerializeField]
         private ItemDefinition item;

        /// <summary>Fallback item name resolved by ItemRegistry when the direct reference is null.</summary>
        [FormerlySerializedAs("_itemName"), Tooltip("Item name (fallback when direct reference not set)"), SerializeField]
         private string itemName = "";

        /// <summary>Weighted-random selection weight relative to other entries in the same pool.</summary>
        [FormerlySerializedAs("_weight"), Tooltip("Selection weight"), Min(1), SerializeField]
         private int weight = 1;

        /// <summary>All conditions must pass for this entry to be eligible during pool rolls.</summary>
        [FormerlySerializedAs("_conditions"), Tooltip("Conditions for this entry"), SerializeField]
         private List<LootConditionEntry> conditions = new();

        /// <summary>Functions applied to the drop after selection (e.g. set count, apply fortune).</summary>
        [FormerlySerializedAs("_functions"), Tooltip("Functions to apply to this entry"), SerializeField]
         private List<LootFunctionEntry> functions = new();
